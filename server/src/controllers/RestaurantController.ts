...
export default class RestaurantController implements Crud {
    ...
    async create(request: Request, response: Response){
        const {name, address, cuisine} = request.body;

        const isAnyUndefined = Citi.areValuesUndefined(name, address, cuisine);
        if(isAnyUndefined) return response.status(400).send();

        const newRestaurant = { name, address, cuisine };
    ...
    async update(request: Request, response: Response){
        ...
        const isAnyUndefined = Citi.areValuesUndefined(name, address, cuisine, id);
        if(isAnyUndefined) return response.status(400).send();

        const restaurantWithUpdatedValues = { name, address, cuisine };
